...
## ...
## ...
# solve for this one - don't sending ports depend on packet destination?
...
## packets_table = ...
# read in and store - in other words, receive and place into structure of choice
packets_table = read_csv('input/packets.csv')
# unsure of this one, may be accepting packets received instead?
...
for packet in packets_table:
    ...
    destinationIP_int = int(ip_to_bin)
    ...
    if ((outPort == "8000") and (new_ttl > 0)):
        ...
        soc.send(new_packet)
        serverData = soc.recv(4096)
        print(serverData.decode())
        # repeat the above as necessary for compatible routers in hierarchy
